#pragma once
#include <cstdint>
#include <cstring>
#include <stdexcept>

// ------------------------------------------------------------
// S-Boxen (Rijndael) 
// ------------------------------------------------------------

const unsigned char sbox[16][16] = {
    
};

const unsigned char inv_sbox[16][16] = {
    , };

extern const unsigned char sbox[16][16];
extern const unsigned char inv_sbox[16][16];

// ------------------------------------------------------------
// AES-Klasse
// ------------------------------------------------------------
class AES
{
public:
    // keyLen erwartet 128, 192 oder 256
    AES(int keyLen = 256);

    // Encrypt/Decrypt mit PKCS#7-Padding, CBC-Modus
    // Gibt einen neuen Puffer zurück, den du löschen musst (delete[])
    // outLen enthält die Größe des Cipher/Plaintext.
    unsigned char* EncryptCBC_PKCS7(const unsigned char* in, unsigned int inLen,
        const unsigned char* key,
        const unsigned char* iv,
        unsigned int& outLen);

    unsigned char* DecryptCBC_PKCS7(const unsigned char* in, unsigned int inLen,
        const unsigned char* key,
        const unsigned char* iv,
        unsigned int& outLen);

private:
    int Nb;
    int Nk;
    int Nr;
    unsigned int blockBytesLen;

    // Kernmethoden
    void KeyExpansion(const unsigned char* key, unsigned char* w);
    void EncryptBlock(const unsigned char* in, unsigned char* out, const unsigned char* roundKeys);
    void DecryptBlock(const unsigned char* in, unsigned char* out, const unsigned char* roundKeys);

    // Hilfsroutinen
    void SubBytes(unsigned char** state);
    void ShiftRow(unsigned char** state, int i, int n);
    void MixSingleColumn(unsigned char* r);
    void ShiftRows(unsigned char** state);
    void MixColumns(unsigned char** state);
    void AddRoundKey(unsigned char** state, const unsigned char* key);
    void InvSubBytes(unsigned char** state);
    void InvShiftRows(unsigned char** state);
    void InvMixColumns(unsigned char** state);
    void XorBlocks(unsigned char* a, unsigned char* b, unsigned char* c, unsigned int len);
    unsigned char xtime(unsigned char b);
    unsigned char mul_bytes(unsigned char a, unsigned char b);
    void printHexArray(unsigned char a[], unsigned int n);
    void Rcon(unsigned char* a, int n);
    void RotWord(unsigned char* a);
    void SubWord(unsigned char* a);
    void XorWords(unsigned char* a, unsigned char* b, unsigned char* c);

    // PKCS#7 Helfer
    unsigned int Pkcs7Pad(const unsigned char* in, unsigned int inLen, unsigned char*& outPadded);
    void RemovePkcs7Padding(unsigned char* data, unsigned int& dataLen);
};

